...
export const authService = {
	findOrCreateUserByEmail(email: string): User {
		...
		db.prepare('INSERT INTO users (id, email, is_admin, created_at) VALUES (@id, @email, @is_admin, @created_at)').run(
			newUser
		);
		return newUser;
	},
	...
	listTokenRequests(status?: TokenRequestStatus): TokenRequest[] {
		...
		if (status) {
			return db
				.prepare<[TokenRequestStatus], TokenRequest>(
	...
	approveTokenRequest(requestId: string, adminNote?: string): { request: TokenRequest; token: Token } {
		const db = getDb();
	...
	rejectTokenRequest(requestId: string, adminNote?: string): TokenRequest {
		const db = getDb();
		const now = new Date().toISOString();
		db.prepare('UPDATE token_requests SET status = ?, admin_note = ?, updated_at = ? WHERE id = ?').run(
			'rejected',
			adminNote || null,
			now,
			requestId
	...
	updateTokenScopes(tokenId: string, apis: string[]): Token {
		const db = getDb();
		db.prepare('UPDATE tokens SET apis = ?, revoked_at = NULL, active = 1 WHERE id = ?').run(apis.join(','), tokenId);
		const token = db
			.prepare<[string], Token>('SELECT * FROM tokens WHERE id = ?')
	...
	revokeToken(tokenId: string): void {
		const db = getDb();
		db.prepare('UPDATE tokens SET active = 0, revoked_at = ? WHERE id = ?').run(new Date().toISOString(), tokenId);
	},
};